...
public class HomePageRwd extends _BasePageRwd {
	...
	public CatalogPageRwd goToCategory(int nCategory) throws PageObjectException {
		try {
			...
			if (menuL0.size() == 0)
				throw new PageObjectException("Le menu n'est pas accessible ou n'existe pas", e);
			if (nCategory > menuL0.size())
				throw new PageObjectException("La catégorie demandée est hors menu", e);
			throw e;
		}
	}
	...
	public int getNumberOfCategories() throws PageObjectException {
		if (menuL0.size() == 0)
			throw new PageObjectException("Le menu n'est pas accessible ou n'existe pas");
		else {

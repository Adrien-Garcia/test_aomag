 ...
 * @since 2015-04-25
 */
public class CheckoutPage extends _BasePage {
	/**
	 * Logger pour cette page
	 */
	private static Log log = LogFactory.getLog(CheckoutPage.class);
	 ...
	 */
	@FindBy(css = ".sel-sel-bill-use_for_ship")
	...
	/**
	 * CheckBox pour utiliser l'adresse de facturation pour livraison
	 */
	@FindBy(css = ".sel-ship-use_billing_add")
	private WebElement billForShip;
	/**
	 * Liste choix méthode livraison
	 */
	@FindBy(css = ".sel-form-method-shipping")
	 ...
	 */
	public CheckoutPage(RemoteWebDriver _driver) throws PageObjectException {
		super(_driver);
	...
	public void checkoutAsGuest() throws PageObjectException {
		try {
			if (isElementPresent(guestCheckout))
				guestCheckout.click();
	...
	public void fillShippingAddress()
			throws PageObjectException {
		if (isElementPresent(billForShip)){
			billForShip.click();
			billForShip.submit();
		}else{
			throw new PageObjectException(this.driver, "adresse de livraison..");
		}
	}
